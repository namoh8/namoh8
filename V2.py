...
bot = telebot.TeleBot("7178780995:AAGVg0_aDGTlPpKaNhg7_Z-oCoTt6loG0Jw")
...
def get(message):
    ...
    r1 = requests.get("https://github.com/namoh8/namoh8/blob/main/aloow.text").text
    if '05' in mobile_number and str(user_id) not in  str(r1):    	
    	tu = types.InlineKeyboardButton('المطور',url='t.me/Dl_Luffy')
    	...
    	return False
    elif str(user_id) in r1 and mobile_number not in r1:
    	bot.send_message(chat_id=message.chat.id, text='عذرا عزيزي هذا الرقم غير مدرج بقائمة الارقام المسموح لها باستعمال البوت')
    elif mobile_number in r1 and    str(user_id)  in r1:
